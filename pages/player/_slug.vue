<template>
  ...
  >
    ...
    <div class="container">
      <div
        class="flex md:flex-row flex-col bg-secondary-100 p-8 md:p-68 rounded-50"
      >
        <div class="md:w-2/5">
          <div
          ...
          >
            <h3 class="text-white font-bold text-lg md:text-2xl uppercase">
              <n-link :to="`/player/${playerData.slug}`">{{
                playerData.name
              }}</n-link>
            </h3>
          ...
          <ul
            class="flex justify-center text-white mt-8 max-w-full md:max-w-sm"
          >
            <li class="mr-2">
              <a
        ...
        <div class="md:w-3/5 md:pl-10 mt-16 md:mt-0">
          ...
          <div class="about_desc mb-10">
            <p class="lg:text-xl lg:leading-8">
...
export default {
  ...
  mounted() {
    this.playerData = playerData.find(
      (player) => player.slug == this.$route.params.slug
    );
  },
};
</script>
